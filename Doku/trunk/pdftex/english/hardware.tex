...
10nF and R8 is moved so that the PD6 output does not try to switch a 100nF capacitor directly.
...
needed, one \(27k\Omega\) resistor is added to the PD7 (pin 12) input. With this modification
...
Every LCD-display with a 2~x~16 character display and a HD44780 compatible controller
...
you should disconnect the power and check all four data and two control signal connections.
...
ATmega are correct set to that speed. You find all parameter data in the corresponding
Makefile.
