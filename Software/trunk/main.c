...
//	disable digital inputs of Analog pins, but TP1-3 digital inputs must be left enabled for VGS measurement
	...
	//     ADC_DDR = TXD_MSK;		//activate Software-UART 
	 ...
	 #endif
#ifdef TEST_SamplingGenerator
//====================================================================
// Test-mode for samplingADC
          uint16_t uu[256];
          uint8_t kk;
 ...
 #define DPW_16 14
          ...
          ADC_DDR =  TXD_MSK; 	// no ADC port output
          ADMUX = TP3|(1<<REFS0)|(1<<REFS1);
          for (kk=0;kk<10;kk++) {
	  lcd_clear();
	  lcd_data('0'+DPW_1);
          for (ii=0; ii<64; ii++) {
            samplingADC((DPW_1<<8)|samplingADC_twopulses, uu, 0, (1<<(TP3*2)), 0, PORT_LC0, (1<<(TP3*2)));
	    wdt_reset();
            if ((ii % 16) == 0) {
	       lcd_line2();
	       DisplayValue16(uu[0],0,',',5);
	       DisplayValue16(uu[1],0,',',5);
	       DisplayValue16(uu[2],0,' ',5);
            }
          }
	  lcd_line1();
	  lcd_data('0'+DPW_2);
          for (ii=0; ii<64; ii++) {
            samplingADC((DPW_2<<8)|samplingADC_twopulses, uu, 0, (1<<(TP3*2)), 0, PORT_LC0, (1<<(TP3*2)));
	    wdt_reset();
            if ((ii % 16) == 0) {
	       lcd_line2();
	       DisplayValue16(uu[0],0,',',5);
	  ...
	  lcd_data('0'+DPW_3);
          for (ii=0; ii<64; ii++) {
            samplingADC((DPW_3<<8|samplingADC_twopulses), uu, 0, (1<<(TP3*2)), 0, PORT_LC0, (1<<(TP3*2)));
	    wdt_reset();
            if ((ii % 16) == 0) {
	       lcd_line2();
	       DisplayValue16(uu[0],0,',',5);
	  ...
	  lcd_line1();
          DisplayValue16(DPW_4,0,' ',4);
          for (ii=0; ii<64; ii++) {
            samplingADC((DPW_4<<8|samplingADC_twopulses), uu, 0, (1<<(TP3*2)), 0, PORT_LC0, (1<<(TP3*2)));
	    wdt_reset();
            if ((ii % 16) == 0) {
	       lcd_line2();
	       DisplayValue16(uu[0],0,',',5);
	  ...
	  lcd_line1();
          DisplayValue16(DPW_8,0,' ',4);
          for (ii=0; ii<64; ii++) {
            samplingADC((DPW_8<<8|samplingADC_twopulses), uu, 0, (1<<(TP3*2)), 0, PORT_LC0, (1<<(TP3*2)));
	    wdt_reset();
            if ((ii % 16) == 0) {
	       lcd_line2();
	       DisplayValue16(uu[0],0,',',5);
	  ...
	  lcd_line1();
          DisplayValue16(DPW_16,0,' ',4);
          for (ii=0; ii<64; ii++) {
            samplingADC((DPW_16<<8|samplingADC_twopulses), uu, 0, (1<<(TP3*2)), 0, PORT_LC0, (1<<(TP3*2)));
	    wdt_reset();
            if ((ii % 16) == 0) {
	       lcd_line2();
	       DisplayValue16(uu[0],0,',',5);
